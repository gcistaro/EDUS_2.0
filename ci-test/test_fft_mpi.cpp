...
/*     ***********************get splitted arrays************************************************/
    ...
    MPIindex<3> mpindex(dims, howmany);
...
/*     ***********************end of get splitted arrays************************************************/
    ...
    mdarray<std::complex<double>,2> Array_x( { LocalRange.second-LocalRange.first+1, howmany  },mpindex.get_RecommendedAllocate_fftw() );//({1,int(N0)*int(N1)});
    mdarray<std::complex<double>,2> Array_k( { LocalRange.second-LocalRange.first+1, howmany  }, mpindex.get_RecommendedAllocate_fftw() );//({1,int(N0)*int(N1)});
#else
    ...
    for( int ihowmany = 0; ihowmany < howmany; ++ihowmany ) {
...
#ifdef EDUS_MPI
            Array_x(oneDindex_loc, ihowmany) =
#else
    ...
    std::cout << "Rank " << irank <<  " Settingv up Fourier Transform..\n";
{
    ...
    os_rank << "+--+---------+------------+------------+------------------------------------------------------+-------------------+-------------------+\n";
    os_rank << "|ih|   1Df   |    freqx   |    freqy   |               Numerical solution                     |Analytical solution|      Error(%)     |\n";
    os_rank << "+--+---------+------------+------------+------------------------------------------------------+-------------------+-------------------+\n";
    ...
    for( int ihowmany = 0; ihowmany < howmany; ++ihowmany ) {
        for( int oneDindex_loc = 0; oneDindex_loc < mpindex.nlocal; ++oneDindex_loc ) {
...
#ifdef EDUS_MPI
            auto NumericalSolution  = Array_k( oneDindex_loc, ihowmany );
#else
            auto NumericalSolution  = Array_k( ihowmany, oneDindex_loc );
#endif
            auto RelativeError = 100*abs(NumericalSolution - AnalyticalSolution )/abs(AnalyticalSolution);
            //print all infos
            os_rank << "|  ";
            os_rank << std::setw(2) << std::fixed << ihowmany;
            os_rank << "|";
            os_rank << std::setw(7) << std::fixed << oneDindex_loc;
            os_rank << "|";
            ...
            os_rank << "|";
            os_rank << "  ";
            os_rank << std::setw(12) << std::setprecision(8) << std::scientific <<  NumericalSolution;
            os_rank << "  ";
            os_rank << "|";
            os_rank << "  ";
            os_rank << std::setw(12) << std::setprecision(8) << std::scientific << AnalyticalSolution;
            os_rank << "   ";
            os_rank << "|";
            os_rank << "  ";
            ...
            if(AnalyticalSolution > 1.e-07 && RelativeError > 0.5){
                std::cout << "Condition \"AnalyticalSolution > 1.e-07 && RelativeError > 0.5\" not satisfied. Error\n";
                exit(1);
            }
        }
        os_rank.close();
