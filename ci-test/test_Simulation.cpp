...
{            
    ...
    auto SourceTerm = [&](Operator<std::complex<double>>& DM_Output, const double& time, const Operator<std::complex<double>>& DM_Input){
        std::fill(DM_Output.get_Operator_R().begin(), DM_Output.get_Operator_R().end(), 0.);
    };

    auto simulation = make_Simulation<Operator<std::complex<double>>>(InitialCondition, SourceTerm);
}