...
template<size_t dim>
MPIindex<dim>::MPIindex( const std::array<size_t, 3>& ValuesToSplit__ )
{
    initialize(ValuesToSplit__ );
}

template<size_t dim>
void MPIindex<dim>::initialize( const std::array<size_t, 3>& ValuesToSplit__ )
{
    ...
    std::cout << "local_0_start " << local_0_start << " local_n0 " << std::endl;
    LocalRange_1D.first  = multindex.oneDindex(local_0_start,0,0);
    LocalRange_1D.second = multindex.oneDindex(local_0_start + local_n0-1,ValuesToSplit[1]-1,ValuesToSplit[2]-1);
...
{
    assert( local >= 0 && local <= (LocalRange_1D.second - LocalRange_1D.first) );
...
{
    return glob1D_to_loc1D( globnD_to_glob1D(args...) );
}

template<size_t dim>
inline std::array<int, dim> MPIindex<dim>::loc1D_to_globnD(const std::ptrdiff_t& loc1D)
{
    return glob1D_to_globnD( loc1D_to_glob1D(loc1D) );
}
