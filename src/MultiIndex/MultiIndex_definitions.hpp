...
template<size_t dim> 
MultiIndex<dim>::MultiIndex(const std::array<size_t,dim>& Size__)
{
    initialize(Size__);
}

template<size_t dim> 
void MultiIndex<dim>::initialize(const std::array<size_t,dim>& Size__) 
{
    Size = Size__;
...
template<size_t dim> 
std::array<int,dim> MultiIndex<dim>::nDindex(const int& index)
{
