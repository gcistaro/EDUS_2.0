...
class Simulation
{
    private:
        ...
        std::array<Operator<std::complex<double>>, 3> Position;
...
{
    assert(O1.get_MeshGrid()->get_id() == O2.get_MeshGrid()->get_id());
    ...
    auto& ci = MeshGrid<R>::ConvolutionIndex1[{Operator<std::complex<double>>::MeshGrid_Null->get_id(), 
                                              ...
                                              Minus_MG->get_id()}];
    if(ci.get_Size(0) == 0 ){
        MeshGrid<R>::Calculate_ConvolutionIndex1(*(Operator<std::complex<double>>::MeshGrid_Null), *(O1.get_MeshGrid()), *Minus_MG);
    }
