...
def read_wannierTB(filename):
    ...
    unit_cell = np.array([[float(fsplit[1][0]),float(fsplit[1][1]),float(fsplit[1][2])],
    ...
    for iR in range(num_R):
        ...
        for irow in range(num_bands):
            for icol in range(num_bands):
                H[iR][irow][icol] = float(fsplit[index][2]) + 1j*float(fsplit[index][3])
                index = index+1             
        index = index+1
    ...
    for iR in range(num_R):
        ...
        for irow in range(num_bands):
            for icol in range(num_bands):
                r[0][iR][irow][icol] = float(fsplit[index][2]) + 1j*float(fsplit[index][3])
                r[1][iR][irow][icol] = float(fsplit[index][4]) + 1j*float(fsplit[index][5])
                r[2][iR][irow][icol] = float(fsplit[index][6]) + 1j*float(fsplit[index][7])
                index = index+1 
